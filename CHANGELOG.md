...
- Fixed last-good Recipe Review fallback detection for personal plans whose authorized calculation config omits display-only plan names.
...
- Fixed saved recipe routes being rejected when current stock fully covered their live recipe branch.
...
- Removed the Featurebase in-app messenger and signed account identity integration.

### Security

- Removed the obsolete Featurebase signing secret and external browser permissions from the application deployment.
