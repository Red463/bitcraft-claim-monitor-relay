...
| `craft_plan_route_reviews` | keep-user | Successful authenticated Craft Plan saves with confirmed ambiguous routes | Current confirmation metadata keyed by plan and exact typed output. Alternative-signature changes selectively invalidate affected rows; plan deletion removes them and account deletion anonymizes retained reviewer identity. |
