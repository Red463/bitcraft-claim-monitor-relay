...
test("preview HTTP routes enforce auth, ownership, CSRF, rate limiting, and never persist staged changes", async (t) => {
  ...
  const setup = await fetch(`${origin}/api/local/admin/setup`, {
    ...
    body: JSON.stringify({ username: "admin", password: "correct horse battery", setupKey: "preview-setup" }),
  });
  const adminAuth = await setup.json();
  const adminCookie = setup.headers.get("set-cookie").split(";")[0];
  const adminPlans = await fetch(`${origin}/api/local/admin/craft-plans`, { headers: { cookie: adminCookie, origin } }).then((response) => response.json());
